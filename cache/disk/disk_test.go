...
// Verifies that Put operations are throttled to 5000 concurrent operations.
func TestPutRequestThrottling(t *testing.T) {
	...
	const numberOfExpectedConcurrentOperations = 5000
	const numberOfAdditionalOperations = 77 // arbitrary choosen
	const numberOfOperations = numberOfExpectedConcurrentOperations + numberOfAdditionalOperations
...
// Verifies that proxy downloads for Get operations are throttled to 5000 concurrent operations.
func TestProxiedGetRequestThrottling(t *testing.T) {
	...
	const maxSize = numberOfOperations * BlockSize
...
// http.StatusInsufficientStorage when they togheter try to reserve
// more space than what is currently available.
