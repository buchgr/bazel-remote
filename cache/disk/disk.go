...
func (c *diskCache) Put(ctx context.Context, kind cache.EntryKind, hasher hashing.Hasher, hash string, size int64, r io.Reader) (rErr error) {
	...
	sizeOnDisk, err = c.writeAndCloseFile(r, kind, hasher, hash, size, tf)
	if err != nil {
		return internalErr(err)
	}
...
func (c *diskCache) writeAndCloseFile(r io.Reader, kind cache.EntryKind, hasher hashing.Hasher, hash string, size int64, f *os.File) (int64, error) {
	closeFile := true
	defer func() {
		if closeFile {
			f.Close()
